...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield exec.exec(`cp /github/workspace/${specFile} ${mySpecFile}`);
            // create directory to match source file - %{name}-{version}.tar.gz of spec file
            yield exec.exec(`mkdir /github/home/rpmbuild/SOURCES/${name}-${version}`);
            // instead of downloading code from github, just copy the current repo and clean it up a little
            yield exec.exec(`cp -r /github/workspace/ /github/home/rpmbuild/SOURCES/${name}-${version}/`);
            yield exec.exec(`rm -rf /github/home/rpmbuild/SOURCES/${name}-${version}/.git/`);
            // Create Source tar.gz file 
            yield exec.exec(`ls -alF /github/home/rpmbuild/SOURCES/`);
            yield exec.exec(`tar czf /github/home/rpmbuild/SOURCES/${name}-${version}.tar.gz -C /github/home/rpmbuild/SOURCES/ ${name}-${version}`);
            yield exec.exec(`rm -rf /github/home/rpmbuild/SOURCES/${name}-${version}/`);
            yield exec.exec(`ls -alF /github/home/rpmbuild/SOURCES/`);
            // Execute rpmbuild , -ba generates both RPMS and SPRMS
            try {
                process.env.__RPMBUILD_WORKSPACE__ = '/github/workspace/';
                yield exec.exec(`rpmbuild -ba ${mySpecFile}`);
            }
            catch (err) {
